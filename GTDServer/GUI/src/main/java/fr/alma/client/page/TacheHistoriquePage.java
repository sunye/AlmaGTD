...
public class TacheHistoriquePage implements IPageIHM {
	...
	public TacheHistoriquePage(List<Tache> tache) {
		data=tache;
	}
	...
	public HorizontalPanel getRecherchePanel() {
		...
		ListStore<TypeChamp> states = new ListStore<TypeChamp>();  
		states.add(ServerData.getTacheChamp());  
		...
		Button rechercher = new Button("Rechercher", new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				dataService.getDataTache(textBox.getText(), combo.getValue().getType(),
						new AsyncCallback<List<Tache>>() {
					public void onFailure(Throwable caught) {
						Error.showLoadError();
					}

					public void onSuccess(List<Tache> result) {
						ServerData.setDataTache(result);
						if (rafrachir) {
							rafrachir=false;
							History.newItem("102");							
						} else {
	...
	public Portal getPortalPanel() {
		...
		portlet.setHeading("Historique - Tache");
		configPanel(portlet);
	...
	private Grid<Tache> createGrid() {
		...
		column = new ColumnConfig();
		column.setId("priorite");
		column.setHeader("Priorité");
		column.setWidth(50);
		...
		ListStore<Tache> store = new ListStore<Tache>();
		store.add(data);

		ColumnModel cm = new ColumnModel(configs);

		RowEditor<Stock> re = new RowEditor<Stock>();
		Grid<Tache> g = new Grid<Tache>(store, cm);
		g.addPlugin(re);
